  ...
  ; (async () => {
    ...
    for (const source of sourceList) {
      ...
      if (!!targetTable.length) {
        ...
        const redirectTarget = await page.$$eval('a', (aTagList: HTMLAnchorElement[]) => (aTagList.find(aTag => aTag.innerText.includes('点击阅览') || aTag.innerText.includes('点击浏览')) || { href: null }).href);
        if (redirectTarget) {
          await page.goto(redirectTarget, {
            waitUntil: 'networkidle2',
            timeout: 60 * 60 * 1000
          });
          const secondPage = await page.$$eval('table', (tableList: HTMLTableElement[]) => {
            const targetTable = tableList.find(table => table.hasAttribute('id'));
            const trList = Array.from(targetTable.querySelectorAll('tr'));
            return trList.reduce<{ title: string, content: { title: string, target: string }[] }[]>((acc, current, index) => {
              const subItems = Array.from(current.querySelectorAll('a'));
              if (subItems.length) {
                const prev = acc.pop() || { title: '', content: [] };
          ...
          for (const secondItem of secondPage) {
            for (const secondContent of secondItem.content) {
              ...
              if (targetTable) {
                ...
                // page.$
                console.log('img --->', secondContent.target)
              }
              // // TODO: 这里为了节省流量，循环到第一个自动退出
              // break;
            }
